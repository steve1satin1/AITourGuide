...
st.register_type_strategy(pydantic.SecretStr, st._text().map(pydantic.SecretStr))
...
st.register_type_strategy(pydantic.StrictStr, st._text())
...
def resolve_json(cls):  # type: ignore[no-untyped-def]
    try:
        ...
        inner = st.recursive(
            base=st.one_of(st.none(), st.booleans(), st.integers(), finite, st._text()),
            extend=lambda x: st.lists(x) | st.dictionaries(st._text(), x),  # type: ignore
        )
...
def resolve_constr(cls):  # type: ignore[no-untyped-def]  # pragma: no cover
    ...
    if cls.regex is None and not cls.strip_whitespace:
        return st._text(min_size=min_size, max_size=max_size)
