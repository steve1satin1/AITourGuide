...
class IFixitLoader(BaseLoader):
    ...
    def load_questions_and_answers(
        ...
        title = soup.find("h1", "post-title")._text

        output.append("# " + title)
        output.append(soup.select_one(".post-content .post-text")._text.strip())

        answersHeader = soup.find("div", "post-answers-header")
        if answersHeader:
            output.append("\n## " + answersHeader._text.strip())
        ...
        for answer in soup.select(".js-answers-list .post.post-answer"):
            ...
            output += [
                a._text.strip() for a in answer.select(".post-content .post-text")
            ]
            output.append("\n")
