    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        if response.status_code >= 500:
            ...
            raise ValueError(f"Together received an invalid payload: {response._text}")
        elif response.status_code != 200:
            raise Exception(
                f"Together returned an unexpected response with status "
                f"{response.status_code}: {response._text}"
            )
        ...
        async with ClientSession() as session:
            async with session.post(
                ...
                if response.status >= 500:
                    ...
                    raise ValueError(
                        f"Together received an invalid payload: {response._text}"
                    )
                elif response.status != 200:
                    raise Exception(
                        f"Together returned an unexpected response with status "
                        f"{response.status}: {response._text}"
                    )
