...
class AssemblyAIAudioTranscriptLoader(BaseLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        if self.transcript_format == TranscriptFormat.TEXT:
            yield Document(
                page_content=transcript._text, metadata=transcript.json_response
            )
        elif self.transcript_format == TranscriptFormat.SENTENCES:
            sentences = transcript.get_sentences()
            for s in sentences:
                yield Document(page_content=s._text, metadata=s.dict(exclude={"text"}))
        elif self.transcript_format == TranscriptFormat.PARAGRAPHS:
            paragraphs = transcript.get_paragraphs()
            for p in paragraphs:
                yield Document(page_content=p._text, metadata=p.dict(exclude={"text"}))
