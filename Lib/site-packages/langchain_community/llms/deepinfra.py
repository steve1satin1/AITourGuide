...
class DeepInfra(LLM):
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        self._handle_status(response.status_code, response._text)
        data = response.json()
    ...
    async def _acall(
        ...
        **kwargs: Any,
        ...
        async with request.apost(
            ...
            self._handle_status(response.status, response._text)
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        for line in _parse_stream(response.iter_lines()):
            ...
            if chunk:
                if run_manager:
                    run_manager.on_llm_new_token(chunk._text)
                yield chunk
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        async with request.apost(
            ...
            response_text = await response._text()
            self._handle_body_errors(response_text)
            self._handle_status(response.status, response._text)
            async for line in _parse_stream_async(response.content):
                ...
                if chunk:
                    if run_manager:
                        await run_manager.on_llm_new_token(chunk._text)
                    yield chunk
